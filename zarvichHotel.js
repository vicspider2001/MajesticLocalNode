...
zarvich.post('/goodbye', (req, res) => {
  ...
  const collectionName = 'checkoutData';
  db.listCollections({ name: collectionName }).next((err, collection) => {
    if (err) {
      throw err;
    }

    if (collection) {
      // The collection already exists, so insert the document
      ...
      // The collection doesn't exist, so create it and insert the document
...
//delete from checkin data
zarvich.delete('/delchkOut/:id',(req,res)=>{
    var id = req.params.id
    db.collection('checkoutData').deleteOne(
        {_id:id},(err,result)=>{
        if(err) throw err;
        res.send(result)
    })
...
zarvich.post('/restaurant', (req, res) => {
  ...
  const collectionName = 'restaurantData';
  db.listCollections({ name: collectionName }).next((err, collection) => {
    if (err) {
      throw err;
    }
...
zarvich.post('/otherSales', (req, res) => {
  ...
  const collectionName = 'revenueData';
  db.listCollections({ name: collectionName }).next((err, collection) => {
    if (err) {
      throw err;
    }
...
zarvich.post('/postRoomChangeReason', (req, res) => {
  ...
  const collectionName = 'roomChangeData';
  db.listCollections({ name: collectionName }).next((err, collection) => {
    if (err) {
      throw err;
    }
...
zarvich.post('/PostTmpRmDep', (req, res) => {
  ...
  const collectionName = 'TempRoomDeposit';
  db.listCollections({ name: collectionName }).next((err, collection) => {
    if (err) {
      throw err;
    }
...
zarvich.post('/dsrTemp', (req, res) => {
    console.log(req.body);
  
    const collectionName = 'dailySalesTemp';
    db.listCollections({ name: collectionName }).next((err, collection) => {
      if (err) {
        throw err;
      }
...
//Check Internet connection before post

zarvich.post('/DSRCloud', async (req, res) => {
    const isConnected = await internetAvailable();
    if (isConnected) {
      db.collection('dailySalesTemp')
        .find()
        .sort({ timestamp: -1 })
        .limit(25)
        .toArray((err, docs) => {
          ...
          // You can now insert the new document and send the response
          db.collection('dailySalesTemp').insertOne(req.body, (err, result) => {
            if (err) throw err;
            // Forward the request to the remote server
            const postToCloud = 'http://ec2-54-163-15-232.compute-1.amazonaws.com:3333/add';
            axios.post(postToCloud, req.body)
              .then((response) => {
      ...
      }, 120000); // Reschedule the post after 2 minutes
    }
});

zarvich.get('/getDepSSales', (req,res)=> {
    var query = {_id:(req.query.id)};
    console.log(req.query.id)
           
    db.collection('guestDepositTemp').find(query).toArray((err,result) => {
        if(err) throw err;
        res.send(result)
    })
})

zarvich.post('/gdepTemp', (req, res) => {
    console.log(req.body);
  
    const collectionName = 'guestDepositTemp';
    db.listCollections({ name: collectionName }).next((err, collection) => {
      if (err) {
        throw err;
      }
...
// zarvich.post('/guestDepOnline', async (req, res) => {
//     const isConnected = await internetAvailable();
//     if (isConnected) {
//       db.collection('guestDepositTemp')
//         .find()
//         .sort({ timestamp: -1 })
//         .limit(10)
//         .toArray((err, docs) => {
...
//           console.log(`Found ${docs.length} documents`);
//           // You can now insert the new document and send the response
//           db.collection('guestDepositTemp').insertOne(req.body, (err, result) => {
//             if (err) throw err;
//             // Forward the request to the remote server
//             const postgstDepToCloud = 'http://ec2-54-163-15-232.compute-1.amazonaws.com:3333/OnlinegdepTemp';
//             axios.post(postgstDepToCloud, req.body)
//               .then((response) => {
//                 console.log(response.data);
//                 // Delete the contents of guestDepositTemp
//                 db.collection('guestDepositTemp').deleteMany({}, (err, result) => {
//                   if (err) throw err;
//                   res.send('Document inserted');
//                 });
//               })
...
zarvich.get('/getOthrSales', (req,res)=> {
    var query = {_id:(req.query.id)};
    console.log(req.query.id)
           
    db.collection('otherSalesTemp').find(query).toArray((err,result) => {
        if(err) throw err;
        res.send(result)
    })
})

// zarvich.post('/OthrSales', (req, res) => {
//     console.log(req.body);
  
//     const collectionName = 'otherSalesTemp';
//     db.listCollections({ name: collectionName }).next((err, collection) => {
//       if (err) {
//         throw err;
//       }
...
// zarvich.post('/othersOnline', async (req, res) => {
//     const isConnected = await internetAvailable();
//     if (isConnected) {
//       db.collection('otherSalesTemp')
//         .find()
//         .sort({ timestamp: -1 })
//         .limit(10)
//         .toArray((err, docs) => {
...
//           console.log(`Found ${docs.length} documents`);
//           // You can now insert the new document and send the response
//           db.collection('otherSalesTemp').insertOne(req.body, (err, result) => {
//             if (err) throw err;
//             // Forward the request to the remote server
//             const postgstDepToCloud = 'http://ec2-54-163-15-232.compute-1.amazonaws.com:3333/OnlineOthers';
//             axios.post(postgstDepToCloud, req.body)
//               .then((response) => {
//                 console.log(response.data);
//                 // Delete the contents of guestDepositTemp
//                 db.collection('otherSalesTemp').deleteMany({}, (err, result) => {
//                   if (err) throw err;
//                   res.send('Document inserted');
//                 });
//               })
...
//Create Database
zarvich.post('/dsrPoolTemp', (req, res) => {
    console.log(req.body);
